  ...
  if ($params{mode} =~ /my::database::size/) {
    @results = $self->{handle}->fetchall_array(q{
      SELECT NVL(type, 'TOTAL') type, tablespace_name, SUM(bytes_used) real_bytes_used,
       SUM(bytes_free) bytes_free, SUM(real_bytes_max) real_bytes_max
      FROM (
        SELECT
  ...
  if ($params{mode} =~ /my::database::size/) {
    ...
    foreach (values %{$self->{results}}) {
      ...
      if ($type eq "TOTAL") {
        ...
        $self->add_perfdata(sprintf "\'%s_used\'=%.2f%s", lc $type, $used / $factor, $units);
        $self->add_perfdata(sprintf "\'%s_allocated\'=%.2f%s", lc $type, ($used + $free) / $factor, $units);
      }
    }
  } elsif ($params{mode} =~ /my::database::activity/) {
  }
